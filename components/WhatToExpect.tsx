...
const benefits = [
  {
    icon: <Activity className="w-6 h-6 lg:w-7 lg:h-7 text-white" />,
    bg: "bg-blue-500",
  ...
  {
    icon: <Target className="w-6 h-6 lg:w-7 lg:h-7 text-white" />,
    bg: "bg-emerald-500",
  ...
  {
    icon: <BookOpen className="w-6 h-6 lg:w-7 lg:h-7 text-white" />,
    bg: "bg-indigo-500",
  ...
  {
    icon: <ShieldCheck className="w-6 h-6 lg:w-7 lg:h-7 text-white" />,
    bg: "bg-slate-700",
    ...
    <section className="py-16 lg:py-24 bg-white">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-10 lg:mb-16">
          <span className="text-blue-600 font-bold uppercase tracking-wider text-xs lg:text-sm">Conteúdo Premium</span>
        ...
        <div className="grid md:grid-cols-2 gap-5 lg:gap-8 max-w-5xl mx-auto">
          {benefits.map((benefit, index) => (
            <div key={index} className="flex gap-4 lg:gap-5 p-6 lg:p-8 rounded-2xl bg-slate-50 border border-slate-100 hover:shadow-xl hover:bg-white hover:border-blue-100 transition-all duration-300 group">
              <div className={`flex-shrink-0 w-12 h-12 lg:w-14 lg:h-14 rounded-xl ${benefit.bg} shadow-lg shadow-blue-900/10 flex items-center justify-center transform group-hover:scale-110 transition-transform duration-300`}>
                {benefit.icon}
              </div>
              <div>
                <h3 className="text-lg lg:text-xl font-bold text-slate-900 mb-2">{benefit.title}</h3>
                <p className="text-slate-600 leading-relaxed text-sm lg:text-base">{benefit.description}</p>
              </div>
            </div>
          ))}
        </div>
